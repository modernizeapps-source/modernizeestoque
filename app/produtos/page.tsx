...
import { podeEditarProdutos } from '@/lib/supabase/auth'
...
function ProdutosConteudo() {
  ...
  const podeEditar = podeEditarProdutos(perfil?.role)
    ...
    <div className="container">
      ...
      <div className="grade-desktop-cards" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {produtosFiltrados.map((p) => {
          ...
          // Funcionário consulta preço e estoque, mas não abre a tela de edição
          // nem enxerga custo e margem — isso é informação do dono.
          const conteudo = (
            <>
              <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                {podeEditar && <span style={{ color: 'var(--text-dim)', fontSize: 16 }}>›</span>}
              </div>
            </>
          )

          const estilo = { display: 'flex', justifyContent: 'space-between', alignItems: 'center', textDecoration: 'none', color: 'var(--text)' }

          return podeEditar ? (
            <Link key={p.id} href={`/produtos/${p.id}`} className="card card-clicavel" style={estilo}>
              {conteudo}
            </Link>
          ) : (
            <div key={p.id} className="card" style={estilo}>
              {conteudo}
            </div>
          )
        })}
