...
export type FaixaHorario = { inicio: number; fim: number }
export type LimitesPeriodo = {
  madrugada: FaixaHorario
  manha: FaixaHorario
  tarde: FaixaHorario
  noite: FaixaHorario
}

export const LIMITES_PADRAO: LimitesPeriodo = {
  madrugada: { inicio: 0, fim: 6 },
  manha: { inicio: 6, fim: 12 },
  tarde: { inicio: 12, fim: 18 },
  noite: { inicio: 18, fim: 24 },
}

function fmtHora(h: number) { return `${String(h).padStart(2, '0')}h` }

function montarPeriodosDia(limites: LimitesPeriodo) {
  return [
    { id: 'madrugada', nome: 'Madrugada', horario: `${fmtHora(limites.madrugada.inicio)}–${fmtHora(limites.madrugada.fim)}`, min: limites.madrugada.inicio, max: limites.madrugada.fim },
    { id: 'manha', nome: 'Manhã', horario: `${fmtHora(limites.manha.inicio)}–${fmtHora(limites.manha.fim)}`, min: limites.manha.inicio, max: limites.manha.fim },
    { id: 'tarde', nome: 'Tarde', horario: `${fmtHora(limites.tarde.inicio)}–${fmtHora(limites.tarde.fim)}`, min: limites.tarde.inicio, max: limites.tarde.fim },
    { id: 'noite', nome: 'Noite', horario: `${fmtHora(limites.noite.inicio)}–${fmtHora(limites.noite.fim)}`, min: limites.noite.inicio, max: limites.noite.fim },
  ]
}
...
function diaSemanaEHoraFortaleza(isoString: string) {
  ...
  const dataSo = new Date(Date.UTC(ano, mes - 1, dia))
  const diaSemana = dataSo.getUTCDay()
...
export function inicioFimPeriodo(periodo: 'hoje' | 'semana' | 'mes'): { inicio: string; fim: string } {
  ...
  let inicioData: Date
  if (periodo === 'hoje') inicioData = hojeUTC
  else if (periodo === 'semana') inicioData = new Date(Date.UTC(ano, mes - 1, dia - diaSemanaHoje))
  else inicioData = new Date(Date.UTC(ano, mes - 1, 1))
...
export async function buscarRelatorio(inicio: string, fim: string, limitesPeriodo: LimitesPeriodo = LIMITES_PADRAO): Promise<Relatorio> {
  ...
  const porCategoria = Object.entries(lucroPorCategoria).map(([nome, lucro]) => ({ nome, lucro })).sort((a, b) => b.lucro - a.lucro)
  ...
  for (const v of vendas ?? []) porForma[v.forma_pagamento] = (porForma[v.forma_pagamento] ?? 0) + v.valor_total
...
export async function buscarEvolucaoDiaSemana(diaSemana: number, numMeses = 7, mesesAtras = 0): Promise<PontoEvolucao[]> {
  ...
  const { data: vendas, error } = await supabase
    ...
    .gte('data_hora', inicioISO).lt('data_hora', fimISO)
  if (error) throw error
